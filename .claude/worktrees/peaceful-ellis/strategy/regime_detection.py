...
# ── [NEW] Parameters ──────────────────────────────────────────────────────────
ADX_PERIOD          = 14    # lookback for ADX / DI smoothing
ADX_TREND_THRESHOLD = 25    # ADX >= 25  →  trending
ATR_PERIOD          = 14    # lookback for ATR (matches mean_reversion ATR_PERIOD)
ATR_SPIKE_WINDOW    = 20    # rolling window used to compute baseline ATR mean
ATR_SPIKE_MULT      = 1.5   # current ATR > 1.5× baseline  →  high_volatility
...
def _atr(df: pd.DataFrame, period: int = ATR_PERIOD) -> pd.Series:
    """Average True Range — identical formula to mean_reversion._atr()."""
    hl = df["high"] - df["low"]
